...
- 目录短链（`?d=`）：同规格随机令牌，默认 7 天 TTL，仅隐藏路径不改变权限；可选 `state_backend`（sqlite/json）跨重启持久化
- 本地目录缓存：列表与 README 本地缓存（内存 + SQLite/JSON），受 TTL 约束；缓存命中时仍强制执行 `allowed_paths` 白名单二次校验，杜绝配置热改导致的数据越权
- 路径校验：拒绝 `..`、异常字符、规范化后语义变化的路径
- 直链域名白名单：仅允许百度相关 HTTPS 域名
- dlink 缓存绑定 User-Agent，降低跨浏览器复用风险
- 图片预览单独走受限接口，仅允许图片 MIME 类型且限制为 16 MB（可配 `preview_max_mb`），流式转发不整块驻留内存；预览数据经过 VPS 并以 `inline` 返回
...
开启鉴权后，复制的 `/d/{token}` 下载链接**仍然需要接收方已登录**（共享同一 `access_token`）。  
短链隐藏的是文件路径，不是访问控制本身。

目录短链（地址栏 `/?d=令牌`）同理：隐藏浏览路径，打开时仍需登录，目录权限在使用时按 `allowed_paths` 重新校验；令牌默认 7 天有效。**未配置 `state_backend` 时（默认）服务重启后全部短链失效**（内存模型）；配置 `sqlite`/`json` 后端后，目录短链与下载短链跨重启保留，仍受 TTL 约束。

## 已知局限

- 未配置 `trusted_proxy_ips` 时，反代后所有用户会被视为同一来源，限流/锁定变成共享桶（见 [deployment.md](deployment.md)）
- 单进程内存短链/缓存/限流状态：多实例不共享；短链可经 `state_backend` 持久化到本机（多实例仍不共享）
